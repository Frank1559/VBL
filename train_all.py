...
train_accuracy1 = np.empty(len(c_svm))
test_accuracy1 = np.empty(len(c_svm))
...
for i, k in enumerate(c_svm):
    ...
    train_accuracy1[i] = svm.score(X_train, y_train)
    # Compute accuracy on the test set
    test_accuracy1[i] = svm.score(X_test, y_test)
...
train_accuracy3 = np.empty(len(var_gnb))
test_accuracy3 = np.empty(len(var_gnb))
...
for i, k in enumerate(var_gnb):
    ...
    # Compute accuracy on the training set
    train_accuracy3[i] = gnb.score(X_train, y_train)
    # Compute accuracy on the test set
    test_accuracy3[i] = gnb.score(X_test, y_test)

# plot all three
plt.subplot(1, 3, 1)
plt.plot(c_svm, test_accuracy1, label='Testing Accuracy')
plt.plot(c_svm, train_accuracy1, label='Training accuracy')
plt.xlabel('C')
plt.ylabel('Accuracy')
...
plt.subplot(1, 3, 3)
plt.plot(var_gnb, test_accuracy3, label='Testing Accuracy')
plt.plot(var_gnb, train_accuracy3, label='Training accuracy')
